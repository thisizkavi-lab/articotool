import { ArrowLeft, ArrowRight, Check, Mic2, Search, Sparkles } from 'lucide-react'
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <section className="grid gap-4 sm:grid-cols-3 mb-10">
          ...
          <div className="rounded-xl border bg-card px-5 py-4">
            <p className="text-2xl font-semibold">Next</p>
            <p className="text-sm text-muted-foreground mt-1">hunt and promote clips</p>
          </div>
        </section>
              ...
              <CardContent className="p-5 flex h-full flex-col">
                ...
                <div className="mt-auto pt-5 border-t border-border/60">
                  <Button variant="outline" size="sm" asChild>
                    <Link href={`/explore?q=${encodeURIComponent(model.searchQuery)}`}>
                      <Search className="h-4 w-4" />
                      Hunt clips in Explore
        ...
        <p className="text-xs text-muted-foreground mt-8 max-w-2xl">
          Clips are intentionally promoted only after transcript and context review. The hunt buttons open ArticoTool&apos;s YouTube search so the corpus can grow from verified, practice-worthy material.
        </p>
      </main>
    </div>
  )
